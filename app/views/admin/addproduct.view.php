							...
							<div class="col-sm-12 col-12">
								<div class="card">
									<div class="card-header">
										<div class="card-title">Thêm món ăn</div>
									</div>
									<div class="card-body">
...
<form class="row g-3 p-5" id="regisForm" action="" method="POST">
  <div class="col-md-6">
    ...
    <input type="text" class="form-control" value="" id="" name="nameProduct" required>
  </div>
  <div class="col-md-6">
    <label for="" class="form-label">Mô tả:</label>
    <input type="text" class="form-control" value="" id="" name="description" required>
  </div>
	<?php if ($_SESSION['user']['idRole_user']==4): ?>
		<div class="col-md-6">
		<label for="" class="form-label">Quán:</label>
		<select name="idCategory" class="form-control">
		<?php
			foreach ($getAllStore1 as $key => $value ) {
				if($key==0){
					echo'<option value="'.$value['id_category'].'" selected>'.$value['nameCategory'].'</option>';
				}else{
					echo'<option value="'.$value['id_category'].'">'.$value['nameCategory'].'</option>';
				}
			}
		?>
		</select>
		</div>
	<?php elseif($_SESSION['user']['idRole_user']==3): ?>
			<input type="text" hidden name="idCategory" value="<?=$_SESSION['user']['belongTo']?>">
	<?php endif;?>
  <!-- <div class="col-md-6">
    <label for="" class="form-label">Quán:</label>
    <select name="idCategory" class="form-control">
  ...
  <div class="col-md-6">
    ...
    <input type="number" class="form-control" value="" min="0" name="original_price" required>
  </div>
  <div class="col-md-6">
    <label for="" class="form-label">Giá bán:</label>
    <input type="number" class="form-control" value="" min="0" name="price" required>
  </div>
  <div class="col-md-6">
    <label for="" class="form-label">Link ảnh:</label>
    <input type="text" class="form-control" value="" id="" name="imageCover" required>
  </div>
  <div class="col-12">
    <button type="submit" class="btn btn-primary" name="add-product-btn">Thêm</button>
  </div>
</form>
										</div>
										<!-- Row end -->
