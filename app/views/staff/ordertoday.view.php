...
<body>
						...
						<div class="col-sm-12 col-12">
							<div class="card">
								...
								if(isset($_POST['idShift_order'])){
									echo'
									<div class="card-title">Đơn hàng phải giao ( '.$getShiftById['nameShift'].')-ngày hôm nay: ' . $currentDate->format('Y-m-d').'</div>
									';
								}else{ if(is_array($getShiftNow)){
									echo'
									<div class="card-title">Đơn hàng phải giao ( '.$getShiftNow['nameShift'].')-ngày hôm nay: ' . $currentDate->format('Y-m-d').'</div>
									';
								}else{
								...
								if(isset($seeOrdersByDateAndShift)){
									...
									echo'
									<div class="d-flex mb-2 " style="justify-content:right"><button type="button" class="btn btn-primary" onclick="switchAll()">Phê duyệt hết ('.count($seeOrdersByDateAndShift).' đơn)</button> <button type="submit" class="btn btn-success" name="save-status-delivery-btn">Lưu trạng thái</button></div>
									';
								?>
												...
												<?php
													foreach ($seeOrdersByDateAndShift as $key => $value) {
														...
														if ($value['delivery_status'] == 1) {
															echo '
																<span class="text-red"><i class="bi bi-x-circle-fill"></i> ' . $value['nameDelivery'] . '</span>
															   ';
														} else {
															echo '
																<span class="text-green"><i class="bi bi-check-circle-fill"></i> ' . $value['nameDelivery'] . '</span>
															   ';
														}
														echo '</td>
															<td>';
															if($value['delivery_status']==1){
																echo'
																<label class="switch">
															<input type="checkbox" class="checkboxstatus" id="modeSwitch" value="2" name="delivery_status'.$key.'">
															<span class="slider"></span>
														</label>
														
														<!-- Hiển thị chế độ hiện tại -->
														<div class="mode off" id="modeStatus">Chưa giao</div>
																';
															}else{
																echo'
																<label class="switch">
															<input type="checkbox" id="modeSwitch" class="checkboxstatus" name="delivery_status'.$key.'" checked>
															<span class="slider"></span>
														</label>
														
														<!-- Hiển thị chế độ hiện tại -->
														<div class="mode on" id="modeStatus">Đã giao</div>
																';
															}
														echo'</td>
														</tr>
	...
	<script>
		...
		checkboxstatus.forEach(checkbox => {
			...
			if (this.checked) {
				modeStatus.textContent = 'Đã giao';
				modeStatus.classList.remove('off');
				modeStatus.classList.add('on');
			} else {
				modeStatus.textContent = 'Đang giao';
